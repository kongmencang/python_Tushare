...
from config import INFO_ANALYES_URL, SCORCE_TABLE_NAME, PERIOD
...
class Calculate(BaseCaculate):
    ...
    def get_profitability_metrics_to_excel(self, period):
        ...
        for i in range(len(period)):
            df.loc[i] = [
                self.ts_code,
                period[i],
                gross_margin[period[i]],
                operating_margin[period[i]],
                profit_margin[period[i]],
                ROE[period[i]],
                ROA[period[i]],
                EBIT[period[i]],
            ]
    ...
    def get_operational_capability_indicators_to_excel(self, period):
        ...
        # 总资产周转率
        total_asset_turnover = self.get_total_asset_turnover_to_excel(
            copy.deepcopy(period)
        )
        # 应收账款周转率
        accounts_receivable_turnover = (
            self.get_accounts_receivable_turnover_ratio_to_excel(copy.deepcopy(period))
        )
        for i in range(len(period)):
            df.loc[i] = [
                self.ts_code,
                period[i],
                stock_turnover[period[i]],
    ...
    def get_solvency_indicators_to_excel(self, period):
        ...
        for i in range(len(period)):
            df.loc[i] = [
                self.ts_code,
                period[i],
                liquidity_ratio[period[i]],
                liquidity_ratio[period[i]],
                interest_protection[period[i]],
                debt_to_asset_ratio[period[i]],
            ]
            dic[period[i]] = {
                "TS股票代码": self.ts_code,
    ...
    def get_growth_capacity_indicators_to_excel(self, period):
        ...
        # 营收增长率
        increase_rate = self.get_increase_rate_of_main_business_revenue_to_excel(
            copy.deepcopy(period)
        )
        # 营业利润增长率
        operating_rofit_growth_rate = self.get_operating_rofit_growth_rate_to_excel(
            copy.deepcopy(period)
        )
        # 净利润增长率
        net_profit_growth_rate = self.get_net_profit_growth_rate_to_excel(
            copy.deepcopy(period)
        )
        # 固定资产增长率
        growth_rate = self.get_growth_rate_of_fixed_assets_to_excel(
            copy.deepcopy(period)
        )
        # 总资产增长率
        total_asset_growth_rate = self.get_total_asset_growth_rate_to_excel(
            copy.deepcopy(period)
        )
        ...
        for i in range(len(period)):
            df.loc[i] = [
                self.ts_code,
                period[i],
                increase_rate[period[i]],
                operating_rofit_growth_rate[period[i]],
                net_profit_growth_rate[period[i]],
                growth_rate[period[i]],
                total_asset_growth_rate[period[i]],
            ]
    ...
    def get_score_to_excel(self, datas, df_name):
        ...
        # 创建一个包含 None 的字典，并添加评分列
        data = {col: [None] for col in columns}
        data["评分"] = [average_score]

        # 创建 DataFrame 并设置数据类型
        total_score_row = pd.DataFrame(data, index=["总分"]).astype(df.dtypes.to_dict())
        # 将总分行添加到原始DataFrame
    ...
    def get_total_score(self):
        ...
        for stn in SCORCE_TABLE_NAME:
            df = PandasDataFormTool.get_df_from_excel_file(
                dir_path=INFO_ANALYES_URL, ts_code=self.ts_code, table_name=stn
            )
    ...
    def get_score_table(self):
        for stn in SCORCE_TABLE_NAME:
            ...
            chart = Chart()
            chart.get_excel_chart(
                df=df, name=self.name, ts_code=self.ts_code, label=stn
            )
