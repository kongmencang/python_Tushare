...
class BaseCaculate(Company):
    ...
    def get_profit_balance_three_agrs_to_excel(self, period, args):
        ...
        df = pd.DataFrame(
            columns=["TS股票代码", "报告期", args[0]],
            index=[i for i in range(len(period) - 1)],
        )
        for i in range(len(period) - 1):
            ...
            arg3 = pd.read_excel(
                self.get_balance_info_to_excel(period=period[i + 1])
            ).loc[0, args[3]]
    ...
    def get_profit_therr_agrs_reduce_devision(self, period, args):
        ...
        for i in range(len(period) - 1):
            ...
            # 取上期的利润表
            arg2 = pd.read_excel(
                self.get_profit_info_to_excel(period=period[i + 1])
            ).loc[0, args[2]]
            arg3 = pd.read_excel(
                self.get_profit_info_to_excel(period=period[i + 1])
            ).loc[0, args[3]]
